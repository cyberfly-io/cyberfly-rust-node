...
impl IpfsStorage {
    ...
    pub async fn delete_file(
        ...
        tracing::info!("Attempting to delete file with metadata hash: {}", metadata_hash);
    ...
    fn verify_metadata_signature(metadata: &FileMetadata) -> Result<()> {
        ...
        tracing::debug!("Metadata signature verified for owner: {}", metadata.owner_public_key);
        Ok(())
    }
...
mod tests {
    ...
    async fn test_ipfs_storage_creation() {
        let temp_dir = std::env::temp_dir().join("iroh_test_create");
        let storage = IpfsStorage::new(&temp_dir, None).await;
    ...
    async fn test_add_bytes() {
        let temp_dir = std::env::temp_dir().join("iroh_test_add");
        let storage = IpfsStorage::new(&temp_dir, None).await.unwrap();
        let data = b"Hello, Iroh!";
        let result = storage.add_bytes(data).await;
        assert!(result.is_ok());
        
        if let Ok(hash) = result {
            // Verify we can retrieve it
            let retrieved = storage.get_bytes(&hash).await;
    ...
    #[tokio::test]
    async fn test_add_and_retrieve_json() {
        let temp_dir = std::env::temp_dir().join("iroh_test_json");
        let storage = IpfsStorage::new(&temp_dir, None).await.unwrap();
        ...
        let hash = storage.store_json(&test_data).await.unwrap();
        let retrieved: TestData = storage.get_json(&hash).await.unwrap();
    ...
    async fn test_file_metadata_serialization() {
        let metadata = FileMetadata {
            ...
            created_at: chrono::Utc::now().timestamp(),
            hash: "bafkreiexamplehash123".to_string(),
            checksum: "abc123".to_string(),
            tags: vec!["test".to_string()],
            description: Some("Test file".to_string()),
            owner_public_key: "8f4b3c2a1e5d7f9b6a8c1d3e5f7a9b2c4d6e8f0a1b3c5d7e9f1a3b5c7d9e1f3a".to_string(),
            signature: "a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b2".to_string(),
        };
        
        let json = serde_json::to_string(&metadata);
        assert!(json.is_ok());
        
        let deserialized: Result<FileMetadata, _> = serde_json::from_str(&json.unwrap());
    ...
    async fn test_mime_type_detection() {
        ...
        let mime = IpfsStorage::detect_mime_type(path);
