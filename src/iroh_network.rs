...
impl IrohNetwork {
    ...
    pub async fn new(config: IrohNetworkConfig) -> Result<Self> {
        ...
        let endpoint = Endpoint::builder()
            .discovery_n0()  // Use n0 DNS discovery
        ...
        // Create blob storage
        let store = FsStore::load(&config.data_dir).await?;
        let blobs = BlobsProtocol::new(&store, None);

        // Create gossip protocol
        let gossip = Gossip::builder().spawn(endpoint.clone());

        // Build protocol router with blobs and gossip
        let router = Router::builder(endpoint.clone())
            .accept(BLOBS_ALPN, blobs.clone())
            .accept(GOSSIP_ALPN, gossip.clone())
            .spawn();

        tracing::info!("Iroh router spawned with BlobsProtocol and Gossip");
        ...
        Ok(Self {
            ...
            sync_sender: None,
            discovered_peers: Arc::new(dashmap::DashMap::new()),
            bootstrap_peers: Vec::new(), // No bootstrap peers when creating own node
        })
    }
    ...
    async fn handle_gossip_event(
        ...
        match event {
            GossipEvent::Received(msg) => {
                let from = msg.delivered_from;
                tracing::debug!("Received gossip message from {} on {} topic", from, topic_type);
